...
{
    public static IApplicationBuilder RunMessagesRepositoryMigrations(this IApplicationBuilder app)
    {
        if (Environment.GetEnvironmentVariable("SKIP_MIGRATIONS") != null) return app;
        var configuration = app.ApplicationServices.GetService<IConfiguration>();
        var connectionString = configuration!.GetConnectionString("BotDb")
                               ?? throw new MessagesRepositoryException("No connection string configured.");
